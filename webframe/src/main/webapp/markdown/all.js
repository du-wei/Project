...
$(function() {
    function f(a, b) {
        localStorage && localStorage.setItem("mymark_" + a, b)
    }
    function d(a, b) {
        return localStorage ? null == localStorage.getItem("mymark_" + a) ? void 0 != b ? b: null: localStorage.getItem("mymark_" + a) : void 0 != b ? b: null
    }
    function l() {
    ...
    function t(a) {
        ...
        if (null != a) {
            ...
            $.extend(h, b);
            n(a);
            $("#mymark-changeSession").append('<option value="' + a + '" selected>' + a + "</option>");
            c.focus()
        }
    }
    function H(a) {
        h[a].buildIn ? alert("内置的不能删除, 其实不应该让你看到删除滴。。。") : (G(a), localStorage && localStorage.removeItem("markContent_" + a), $("#mymark-changeSession option").filter(function() {
            return $(this).val() == e
        }).remove(), m--, n("mymark简介"))
    }
    function n(a) {
    ...
    function u(a) {
        $("#mymark-docname").text(a);
        a = $(window).width() - $("#mymark-docname").outerWidth(!0);
        $("#mymark-docname").css("left", a / 2)
    }
    function o(a) {
        window.iframeDoc = g;
    ...
    function C(a) {
        $("#mymark-dragMask").size() || $('<div id="mymark-dragMask"></div>').appendTo($("body"));
        $("#mymark-dragMask").show().css({
            left: 0,
            top: 0,
            width: $(window).width(),
        ...
        150 < a.pageX && 150 < $(window).innerWidth() - a.pageX && ($("#mymark-spliter").css("left", a.clientX), window.getSelection ? window.getSelection().removeAllRanges() : document.selection.empty())
    }
    function D() {
        $(document).off("mousemove", C);
        $(document).off("mouseup", D);
        $("#mymark-dragMask").hide();
        var a = $("#mymark-spliter").css("left");
        $("#mymark-editor").css("width", a);
        $("#mymark-result").css("width", $(window).innerWidth() - parseInt(a));
        $("#mymark-result").css("marginLeft", a);
        f("left", a)
    }
    var c = ace.edit("mymark-editor");
    ...
    h = {
        "Editor简介": {
            buildIn: !0,
            content: '![markdown](markdown-logo.png)\n##该markdown编辑器有哪些功能？\n\n* 方便的`导入导出`功能\n    *  直接把一个markdown的文本文件拖放到当前这个页面就可以了\n    *  导出为一个html格式的文件，样式一点也不会丢失\n* 编辑和预览`同步滚动`，所见即所得（右上角设置）\n* `VIM快捷键`支持，方便vim党们快速的操作 （右上角设置）\n* 强大的`自定义CSS`功能，方便定制自己的展示\n* 有数量也有质量的`主题`,编辑器和预览区域\n* 完美兼容`Github`的markdown语法\n* 预览区域`代码高亮`\n* 所有选项自动记忆\n\n'
        },
        "Markdown语法参考": {
            buildIn: !0,
    ...
    $("#mymark-keyboard").prop("checked", parseInt(d("keyboardBinding", 0)));
    x(parseInt(d("keyboardBinding", 0)));
    $("#mymark-linenum").prop("checked", null !== d("lineNum") ? parseInt(d("lineNum")) : 1);
    y($("#mymark-linenum").prop("checked") ? 1 : 0);
    $("#mymark-syncRoll").prop("checked", parseInt(d("curSyncRoll", 0)));
    $("#mymark-editor").css("width", d("left", "50%"));
    $("#mymark-result").css("width", d("left") ? $(window).innerWidth() - parseInt(d("left")) : "50%");
    $("#mymark-result").css("marginLeft", d("left", "50%"));
    $("#mymark-spliter").css("left", d("left", "50%"));
    w(p);
    $(k).on("load",
    function() {
        g = k.contents();
    ...
    function(a, b) {
        $("#mymark-editorThemes").append('<option value="' + b + '" ' + (b == p ? "selected": "") + ">" + b + "</option>")
    });
    $.each(["Clearness-Dark", "Clearness", "GitHub", "GitHub2"],
    function(a, b) {
        $("#mymark-previewThemes").append('<option value="' + b + '" ' + (b == F ? "selected": "") + ">" + b + "</option>")
    });
    null == d("curSession") && (f("curSession", "untitled"), s("untitled"), e = "untitled", null == j(e) && f("markContent_" + e, '![markdown](markdown-logo.png)\n##mymark有哪些功能？\n\n* 方便的`导入导出`功能\n    *  直接把一个markdown的文本文件拖放到当前这个页面就可以了\n    *  导出为一个html格式的文件，样式一点也不会丢失\n* 编辑和预览`同步滚动`，所见即所得（右上角设置）\n* `VIM快捷键`支持，方便vim党们快速的操作 （右上角设置）\n* 强大的`自定义CSS`功能，方便定制自己的展示\n* 有数量也有质量的`主题`,编辑器和预览区域\n* 完美兼容`Github`的markdown语法\n* 预览区域`代码高亮`\n* 所有选项自动记忆\n\n'));
    e = d("curSession");
    u(e);
    $.each(l(),
    function(a, b) {
    ...
    $.each(h,
    function(a) {
        $("#mymark-changeSession").append('<option value="' + a + '" ' + (a == e ? "selected": "") + ">" + a + "</option>")
    });
    $("#mymark-keyboard").click(function() {
        x($(this).prop("checked"))
    });
    $("#mymark-linenum").click(function() {
        y($(this).prop("checked") ? 1 : 0)
    });
    $("#mymark-syncRoll").click(function() {
        q = $(this).prop("checked") ? 1 : 0;
        f("syncRoll", q)
    });
    $("#mymark-editorThemes").change(function() {
        w(this.value)
    });
    $("#mymark-changeSession").change(function() {
        n(this.value)
    });
    $("#mymark-previewThemes").change(function() {
        "custom" == this.value ? showCustomCSSEditor() : v(this.value)
    });
    $("#mymark-removeSession").click(function() {
        H(e)
    });
    $("#mymark-toggleMode").click(function() {
        ...
        c.focus();
        f("cssMode", i);
        $(this).val(i ? "Back Edit": "Custom CSS")
    });
    $("#mymark-attachInfo").click(function() {
        E = $(this).prop("checked")
    });
    $("#mymark-config").click(function() {
        $("#mymark-toolbar").slideDown()
    });
    $("#mymark-closeConfig").click(function() {
        $("#mymark-toolbar").slideUp()
    });
    $("#mymark-session").click(function() {
        t()
    });
    $("#mymark-exportHTML").click(function() {
        var a = prompt("保存的文件名", e),
        b = new BlobBuilder;
        if (a !== null) {
            E && o(c.getSession().getValue() + "\nEdit By [mymark](http://mymark.jser.me)");
    ...
    $("#mymark-importSource").click(function() {
        alert("请直接把文件拖放到此页面上")
    });
    $("#mymark-donate").click(function() {
        $("#mymark-donate-detail").slideDown()
    });
    $("#mymark-closeDonate").click(function() {
        $("#mymark-donate-detail").slideUp()
    });
