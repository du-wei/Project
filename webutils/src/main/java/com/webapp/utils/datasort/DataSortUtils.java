...
public class DataSortUtils {
	...
	public void testName() throws Exception {
		...
		int topSize = top.size() - 1;

		for (int i = 0; i < target.length; i++) {
			for (int k = topSize; k >= 0; k--) {
				if (top.get(k) < target[i]) {
					int temp = top.get(k);
					top.set(k, target[i]);
					if (k != topSize) {
						top.set(k + 1, temp);
	...
	public static <T extends Comparable<T>> void sort(T[] data) {
		sort(data, OrderType.binaryInsert_asce);
	}
	...
	public static <T extends Comparable<T>> T[] shellSort(T[] data) {
		return shellSort(data, null);
	}

	// 插入排序->折半插入排序 稳定
	public static <T extends Comparable<T>> T[] binaryInsertSort(T[] data) {
		return binaryInsertSort(data, null);
	}

	// 插入排序->直接插入排序 稳定
	public static <T extends Comparable<T>> T[] insertSort(T[] data) {
		return insertSort(data, null);
	}

	// 交换排序->快速排序 不稳定
	public static <T extends Comparable<T>> T[] quickSort(T[] data) {
		return quickSort(data, null);
	}

	// 交换排序->冒泡排序 稳定
	public static <T extends Comparable<T>> T[] bubbleSort(T[] data) {
		return bubbleSort(data, null);
	}

	// 选择排序 ->直接选择排序->已改进 不稳定
	public static <T extends Comparable<T>> T[] selectSort(T[] data) {
		return selectSort(data, null);
	}

	// 选择排序 ->堆排序 不稳定
	public static <T extends Comparable<T>> T[] heapSort(T[] data) {
