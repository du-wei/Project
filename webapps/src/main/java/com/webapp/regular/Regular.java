...
public class Regular {
	 ...
	 * (?=k)king(?=k)kin(?=g) 并且g不算在group(0)里 (.{3})(?!g)
	 * 相当于组(.{3})的后面一定不是g，并且g不算在group(0)里 (?!g)(.{3})
	 * 相当于组(.{3})的开头一定不是g，并且g不算在group(0)里 (?!x)所在的位置不能为x 并且x不算在group(0)里
	 * (.{3})(?<=g) 相当于组(.{3})的结尾必须是g，并且算上g一共为3位 (?<=g)(.{3})
	 * 相当于组(.{3})的前面必须是g，并且g不算在group(0)里 (.{3})(?<!g)
	 * 相当于组(.{3})的结尾一定不是g，并且g不算在group(0)里 (?<!g)(.{3})
	 * 相当于组(.{3})的前面一定不是g，并且g不算在group(0)里 ?<= ?<! 使用 从尾开始看(?<=在后) (?<=在前)开头的不算
	 * ?: ?= ?! ?<= ?<! 组不被捕获，也就是没有group(2)， (?i) (?i:java)
	 */
